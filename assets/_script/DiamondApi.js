...
var DiamondApi = {
   ...
   */
  consume: function (amount, onSuccess, onFail) {
    ...
    xhr.send(JSON.stringify({ sessionToken: sessionToken, amount: amount }));
  },
   ...
   */
  serverSync: function (amount, onCorrect) {
    var mgr = $z1BPPayMgr.BPPayMgr.getInstance();
    var sessionToken = mgr.sessionToken;
    var gameServerUrl = mgr.gameServerUrl;
    if (!sessionToken || !gameServerUrl) { return; }
    ...
    xhr.onreadystatechange = function () {
      ...
      } catch (e) { /* 网络异常，忽略，不影响游戏 */ }
    };
    xhr.send(JSON.stringify({ sessionToken: sessionToken, amount: amount }));
  },

  _openShop: function () {
    $z1UIMgr.UIMgr.getInstance().openUI(
      $z1KinghtFallConfig.KinghtFallUIID.UIBPShop,
      $z1KinghtFallConfig.KinghtFallUIID.UIBPShop
    );
  },
};
