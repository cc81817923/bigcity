...
var def_KinghtFallUIGame = function (t) {
  ...
  _ctor.prototype.start = function () {
    ...
    this.initBtnEvent();
    this.initEvent();
    // btnBuildInfo 的 labNum 在 prefab 中居中，左边缘超出屏幕，右移修正
    var buildInfoLabel = this.btnBuildInfo && this.btnBuildInfo.getChildByName("labNum");
    if (buildInfoLabel) { buildInfoLabel.setPosition(24, buildInfoLabel.y); }
    ...
    this.loadPrefab($z1KinghtFallConfig.KinghtFallBundelName.IconMap, this.ctrGame.gameData.levelCfg.prefab, function (e) {
      ...
      t.checkCurrentTaskAlreadyDone();
      // 钻石余额显示：克隆硬币节点并排放在右侧，换成钻石图标
  ...
  _ctor.prototype.refreshDiamondDisplay = function () {
    ...
    if (this._ndDiamondDisplay) {
      ...
      if (lab) { lab.getComponent(cc.Label).string = "" + diamond; }
    }
  };
  ...
  _ctor.prototype.onRefreshDiamond = function (amount, isAdd) {
    ...
    if (!isAdd && amount > 0 && this.btnAddCoin && this.btnAddCoin.active) {
      ...
      cc.tween(label)
        .to(0.9, { position: cc.v3(localPos.x, localPos.y + 90, 0), opacity: 0 })
  ...
  _ctor.prototype.initView = function () {
    ...
    if (this.ctrGame.gameData.taskInfo.index >= this.ctrGame.gameData.taskInfo.list.length) {
      ...
      if (rewardNode && reward) {
        ...
        // labGet / labNoFinish 与 ndAni 在 prefab 里 x 坐标重叠，在此分开：
        ...
        if (isFinished) {
          ...
          if (labGet) { labGet.setPosition(statusX, labGet.y); }
        } else {
  ...
  _ctor.prototype.checkCurrentTaskAlreadyDone = function () {
    ...
    var needLevel = 1;
    switch (taskType) {
      case E.BuildHome:       needBuildType = B.PrivateHouse; needLevel = 1; break;
      case E.BuildTower:      needBuildType = B.ArrowTower;   needLevel = 1; break;
      case E.BuildWall:       needBuildType = B.Wall;          needLevel = 1; break;
      case E.BuildMill:       needBuildType = B.Mill;          needLevel = 1; break;
      case E.BuildBarracks:   needBuildType = B.Barracks;      needLevel = 1; break;
      case E.UpgradeHome:     needBuildType = B.PrivateHouse; needLevel = 2; break;
      case E.UpgradeTower:    needBuildType = B.ArrowTower;   needLevel = 2; break;
      case E.UpgradeWall:     needBuildType = B.Wall;          needLevel = 2; break;
      case E.UpgradeMill:     needBuildType = B.Mill;          needLevel = 2; break;
      case E.UpgradeBarracks: needBuildType = B.Barracks;      needLevel = 2; break;
      case E.UpgradeCastle:   needBuildType = B.CastleCenter;  needLevel = 2; break;
      default: return;
    }
